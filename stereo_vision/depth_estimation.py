...
B = 7.5  # Distance between the cameras [cm]
f = 5  # Camera lens's focal length [mm]
...
with mp_facedetector.FaceDetection(min_detection_confidence=0.5) as face_detection:
    while cap_right.isOpened() and cap_left.isOpened():
        ...
        # If you cannot catch any frame, break
        if not success_right or not success_left:
            ...
            if not results_right.detections or not results_left.detections:
                ...
                cv2.putText(frame_right, "Distance: " + str(round(depth, 1)), (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.2,
                            (0, 255, 0), 3)
                cv2.putText(frame_left, "Distance: " + str(round(depth, 1)), (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.2,
                            (0, 255, 0), 3)
